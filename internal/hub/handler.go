...
func (s *HubServer) handleHeartbeat(w http.ResponseWriter, r *http.Request) {
	...
	device.Status.ReportedFirmwareVersion = req.CurrentFirmwareVersion
	device.Status.LastSeenTime = &metav1.Time{Time: time.Now()}
	device.Status.Phase = iotv1alpha1.DevicePhaseOnline // Mark as online
