...
# ------------------------------------------------------------------------------
#   - KUSTOMIZE, OPERATOR_SDK, OPM, IMG, BUNDLE_IMG, CATALOG_IMG, VERSION,
#   - CONTAINER_TOOL, IMAGE_TAG_BASE
...
readonly OPERATOR_SDK="${OPERATOR_SDK:-${PROJECT_ROOT}/bin/operator-sdk}"
...
generate_bundle() {
    ...
    local kustomize_path="config/components/${component_name}/manifests"
    local img_tag="${IMG:-${IMAGE_TAG_BASE}-${component_name}:v${VERSION}}"
...
build_bundle_image() {
    local component_name="$1"
    local bundle_img_tag="${BUNDLE_IMG:-${IMAGE_TAG_BASE}-${component_name}-bundle:v${VERSION}}"
...
build_catalog_image() {
    local catalog_img_tag="${CATALOG_IMG:-${IMAGE_TAG_BASE}-catalog:v${VERSION}}"
